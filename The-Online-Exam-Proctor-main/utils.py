...
class MockFaceRecognition:
    def face_locations(self, image):
        # Returns a list of tuples with the face locations
        return []
    
    def face_encodings(self, image, face_locations=None):
        # Returns an encoding of the face
        return []
    ...
    def face_distance(self, face_encodings, face_to_compare):
        # Returns an array of face distances
    ...
    def load_image_file(self, file_path):
        ...
        try:
            ...
            return np.zeros((100, 100, 3), dtype=np.uint8)  # Return a dummy image

# Create a global face_recognition mock
...
#ED Related
try:
    with open("utils/coco.txt", "r") as my_file:  # opening the file in read mode
        data = my_file.read()  # reading the file
        class_list = data.split("\n")  # replacing end splitting the text | when newline ('\n') is seen.
except FileNotFoundError:
    # Fallback: create a minimal class list with just the important classes for our app
    print("Warning: utils/coco.txt not found. Using default class list.")
    class_list = ["person", "laptop", "cell phone", "remote", "keyboard", "mouse", "book"]
detected_things = []
...
def move_file_to_output_folder(file_name, folder_name='OutputVideos'):
    ...
    dest_folder = os.path.join(static_dir, folder_name)
    ...
    try:
        ...
        if not os.path.exists(source_path):
            print(f"Error: File '{file_name}' not found at {source_path}")
            return False
            
        # Use 'shutil.move' to move the file to the destination folder
        shutil.move(source_path, destination_path)
        print(f"File '{file_name}' moved to '{folder_name}' folder")
        return True
    except FileNotFoundError:
...
class FaceRecognition:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"Error during FaceRecognition initialization: {e}")
    ...
    def encode_faces(self):
        try:
            ...
            profile_images = []
            try:
                profile_images = os.listdir(profiles_dir)
            except Exception as e:
                print(f"Error listing profiles directory: {e}")
                
            if not profile_images:
                print("No profile images found in the Profiles directory")
                return
            ...
            for image in profile_images:
                try:
                    image_path = f"{profiles_dir}/{image}"
                    ...
                    # Try using OpenCV first to verify the image is valid
                    img = cv2.imread(image_path)
                    if img is None or img.size == 0:
                        print(f"Error: Image {image_path} is empty or invalid")
                        continue
                        
                    # Now try with face_recognition
                    try:
                        face_image = face_recognition.load_image_file(image_path)
                        if face_image is None or face_image.size == 0:
                            print(f"Error: face_recognition failed to load {image}")
                            continue
                    except Exception as e:
                        print(f"Error loading image with face_recognition: {e}")
                        continue
                    ...
                    print(f"Successfully encoded face for {image}")
                except Exception as e:
                    print(f"Error processing image {image}: {str(e)}")
                    
            print(f"Total faces encoded: {len(self.known_face_names)}")
            print(f"Encoded faces: {self.known_face_names}")
        except Exception as e:
            print(f"Error in encode_faces: {str(e)}")
    ...
    def run_recognition(self):
        ...
        # Keep trying to reconnect the camera if needed
        while Globalflag:
            try:
                if not cap or not cap.isOpened():
                    print("Camera not available for face recognition")
                    time.sleep(1)
                    continue
                    
                ret, frame = cap.read()
                if not ret:
                    print("Failed to get frame for face recognition")
                    time.sleep(0.5)
                    continue
                ...
                if self.process_current_frame and len(self.known_face_encodings) > 0:
                ...
                for (top, right, bottom, left), name in zip(self.face_locations, self.face_names):
                    ...
                    try:
                        ...
                        print(f"Error drawing face rectangle: {e}")
        ...
        print("Face recognition thread exiting")
